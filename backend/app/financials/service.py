...
def _as_ratio_value(value: float | None) -> RatioValue:
    ...
    """
    if value is None:
        return RatioValue(value=None, computable=False, reason="net_assets not filed")
...
def extract_financials(profile: ApplicantProfile) -> list[FinancialExtract]:
    ...
    for doc in documents:
        ...
        balance_sheet, profit_and_loss, confidence = parse_ixbrl_document(doc.raw_bytes)
        extracts.append(
            FinancialExtract(
                company_number=profile.company_number,
                filing_year=doc.filing_year,
...
def compute_ratios(extracts: list[FinancialExtract]) -> RatioSet:
    ...
    return RatioSet(
        ...
        gearing=gearing(bs.long_term_liabilities, bs.current_liabilities, bs.shareholder_funds),
        altman_z=altman_z(
